...
testplot <- ggplot(final, aes(x = cand_name, y = support, fill=party)) +
  ...
  scale_y_reverse() +
  coord_flip()+
  scale_y_continuous(breaks=c(0, 1000000,2000000,3000000), labels=c("0", "1M" , "2M", "3M"), expand=c(0,0))+
...
testplot2 <- ggplot(final, aes(x = cand_name, y = general_votes, fill=party)) +
  ...
  theme(axis.title.x = element_text(),
        ...
        legend.position = c(.05, .4)) +
  coord_flip() +
  scale_y_continuous(breaks=c(0,100000,200000, 300000), labels=c("0", "100K", "200K", "300K"), expand=c(0,0))+
  ylab("General Election Votes")+
ylim(450000,0)+
  scale_fill_manual(values=c("#232066", "#E91D0E"), name="Party", breaks = c("D", "R"), labels=c("Democrat", "Republican"))

title <- ggdraw() + draw_label("Relationship Between Top General Election Vote Candidates and Supporting Donations", fontface='bold', size=12)
